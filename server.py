...
def read_messages(r, clients):
    ...
    for sock in r:
        try:
            message = utils.convert_bytes_to_str(sock.recv(1024))
...
def send_messages(messages, w, clients):
    for sock in w:
        for message in messages:
            try:
                sock.send(utils.convert_str_to_bytes(message))
            except:
                sock.close()
                clients.remove(sock)
...
def mainloop():
    with utils.create_tcp_server('', utils.PORT, 17) as s:
        clients = []
        while True:
